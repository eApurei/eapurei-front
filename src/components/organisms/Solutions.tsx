  ...
  return (
    <div className="flex w-full flex-col items-center justify-center gap-4 bg-white p-8 dark:bg-grey-100 lg_2:px-2">
      ...
      <nav className="mt-8">
        <ul className="flex select-none flex-wrap items-center justify-center gap-4 lg_1:gap-2">
          {solutions.map(({ icon: Icon, label, weight }, index) => (
            <li
              key={index}
              className={`${activeIndex === index ? 'dark:text-secondary-150 bg-secondary-100 text-white dark:bg-white' : ''} flex cursor-pointer items-center justify-center gap-2 rounded-md border border-secondary-100 px-4 py-2 font-bold text-secondary-100 duration-300 hover:bg-secondary-100 hover:text-white dark:border-white dark:text-white hover:dark:bg-white hover:dark:text-secondary-100 lg_1:px-2 lg_1:text-sm lg_2:py-1 lg_2:text-xs`}
      ...
      <div className="flex w-full flex-col items-center justify-center">
        ...
        <p
          className={`w-[70%] rounded-md bg-grey-950 p-4 text-center text-base font-bold dark:bg-dark-200 lg_2:text-xs ${roboto.className}`}
        >
